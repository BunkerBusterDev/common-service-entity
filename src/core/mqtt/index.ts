...
let cacheTTL: number;
const mqttApp = new koa();
const port = config.proxyInfo.useProxyMqttPort === undefined ? '1883' : config.proxyInfo.useProxyMqttPort;
let proxyMqttClient: mqtt.MqttClient;
const httpResponseQueue: Record<string, koa.DefaultContext> = {};
const messageCache: Record<string, Record<string, string | number>> = {};
// const messageCache: { [key: string]: { [key: string]: string | number } } = {};
const responseMqttRequestIdentifieriArray: Array<string> = [];

const requestSubscription = () => {
    const logger = global.getLogger();
    const logCategory = global.getLogCategory('proxyMqtt', 'requestSubscription');
...
const registerRequestSubscription = () => {
    const logger = global.getLogger();
    const logCategory = global.getLogCategory('proxyMqtt', 'requestSubscription');
...
const mqttResponse = (responseTopic: string, rsc: number, op: string, to: string, rqi: string, inpc: string) => {
...
const mqttBinding = (op: string, to: string, fr: string, rqi: string, ty: string, pc: string) => {
    return new Promise<{ res: http.IncomingMessage; resBody: string }>((resolve) => {
        ...
        const req = http.request(options, (res) => {
            ...
            res.on('data', (chunk) => {
                bodyStr += chunk;
                console.log('asdfasdfasdf' + chunk);
            });
...
const mqttMessageAction = async (
    ...
    if (jsonObject['m2m:rqp'] !== null) {
        console.log(jsonObject['m2m:rqp']);
...
const mqttMessageHandler = async (topic: string, message: Buffer | string) => {
    ...
    if (
        ...
        try {
            ...
            if (cacheKey in messageCache) {
                ...
                await mqttMessageAction(topicArray, jsonObject);
            }
        } catch {
            let responseTopic = '/oneM2M/resp/';
            responseTopic += topicArray[3] + '/' + topicArray[4] + '/' + topicArray[5];
...
const initialize = (clusterId?: number) => {
    const logger = global.getLogger();
    const logCategory = global.getLogCategory('proxyMqtt', 'listen');
        ...
        http.createServer(mqttApp.callback()).listen({ port: port, agent: false }, async () => {
            ...
            if (resCode === '2000') {
                ...
                if ('m2m:cb' in jsonObject) {
                    ...
                    if (proxyMqttClient === null || proxyMqttClient === undefined) {
                        proxyMqttClient = mqtt.connect(
                            `mqtt://${config.mqttInfo.useMqttBroker}:${config.mqttInfo.useMqttPort}`,
                        );

                        proxyMqttClient.on('connect', () => {
                            requestSubscription();
